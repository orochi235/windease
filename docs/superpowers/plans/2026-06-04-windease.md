...
git commit -m "feat(core): WindeaseStore window lifecycle"
```
...
- [ ] **Step 3: Add methods to `WindeaseStore` in `store.ts`**
...
- [ ] **Step 3: Add methods to `WindeaseStore` in `store.ts`** (place above `// ---- Reactive ----`):
...
- [ ] **Step 3: Add method to `WindeaseStore`**
...
Add methods inside `WindeaseStore` (above `// ---- Internals ----`):
...
**Files:**
- Test: `packages/react/src/WindeaseProvider.test.tsx`
- Create: `packages/react/src/WindeaseProvider.tsx`, `packages/react/src/hooks.ts`
...
- [ ] **Step 3: Implement `packages/react/src/WindeaseProvider.tsx`**
...
export function WindeaseProvider(props: Props): JSX.Element {
  const store = useMemo(() => {
...
```bash
git add packages/react/src/WindeaseProvider.tsx packages/react/src/hooks.ts packages/react/src/WindeaseProvider.test.tsx
