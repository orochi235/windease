...
`hints.aspect` and `justified` are built.** The phase table at the end is the record of what exists.
...
| 3 | Grid tracks, `compact`; `reorder`; stack `tabs`/`side`; desktop `iconFrom`, `wrap`; floating `snap`; `strict` split | `iconFrom`, `wrap`, stack `tabs`/`side` and floating `snap` built; the rest unbuilt |
