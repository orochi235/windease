...
2–4 are unbuilt except `layer`, `resize`, `iconFrom` and `wrap`.** The phase table at the end is
...
| 2 | `tear`, `zoom`, `view` (pan/zoom, desktop fit), `sticky`, `step`, `overshoot`, `layer`, `resize` | `layer` (desktop, floating) and `resize` (desktop) built; the rest unbuilt |
| 3 | Grid tracks, `compact`; stack `tabs`/`side`; desktop `iconFrom`, `wrap`; floating `snap`; `strict` split | `iconFrom` and `wrap` built; the rest unbuilt |
