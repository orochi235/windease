...
| 3 | Grid tracks, `compact`; `reorder`; stack `tabs`/`side`; desktop `iconFrom`, `wrap`; floating `snap`; `strict` split | grid `tracks` and `compact` are built (no preset sets them yet); the rest is unbuilt |
