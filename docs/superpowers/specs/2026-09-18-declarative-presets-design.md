...
| 4 | Packer `rotate`/`sort`/height bound; `hints.aspect`; `justified` strategy; product-look CSS for every preset | in progress: `hints.aspect` and `justified` are built, the rest is not |
