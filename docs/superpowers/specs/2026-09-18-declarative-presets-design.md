...
except grid `sticky`; of phases 3–4, `reorder`, `iconFrom`, `wrap`, `hints.aspect` and
...
| 3 | Grid tracks, `compact`; `reorder`; stack `tabs`/`side`; desktop `iconFrom`, `wrap`; floating `snap`; `strict` split | `reorder`, `iconFrom` and `wrap` built; the rest unbuilt |
