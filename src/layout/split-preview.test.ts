...
const slot: Rect = { x: 100, y: 40, z: 0, w: 300, h: 200 };
...
describe('splitPreviewPlacements', () => {
  it('halves the slot on the split axis and offsets by its origin', () => {
    ...
    expect(out?.get('d')).toEqual({ x: 100, y: 40, z: 0, w: 300, h: 100 });
    expect(out?.get('b')).toEqual({ x: 100, y: 140, z: 0, w: 300, h: 100 });
  });

  it("puts the source second on edge 'end'", () => {
    const out = splitPreviewPlacements(slot, 'd', split({ edge: 'end' }), strip);
    expect(out?.get('b')).toEqual({ x: 100, y: 40, z: 0, w: 300, h: 100 });
    expect(out?.get('d')).toEqual({ x: 100, y: 140, z: 0, w: 300, h: 100 });
  });

  it('splits across the x axis when the intent says so', () => {
    const out = splitPreviewPlacements(slot, 'd', split({ axis: 'x' }), strip);
    expect(out?.get('d')).toEqual({ x: 100, y: 40, z: 0, w: 150, h: 200 });
    expect(out?.get('b')).toEqual({ x: 250, y: 40, z: 0, w: 150, h: 200 });
  });
  ...
  it('honors the prospective group config, so the preview matches the commit', () => {
    ...
    expect(out?.get('d')).toEqual({ x: 100, y: 40, z: 0, w: 300, h: 95 });
    expect(out?.get('b')).toEqual({ x: 100, y: 145, z: 0, w: 300, h: 95 });
  });

  it('places both children even when the slot is too small to halve cleanly', () => {
    const out = splitPreviewPlacements({ x: 0, y: 0, z: 0, w: 10, h: 3 }, 'd', split(), strip);
